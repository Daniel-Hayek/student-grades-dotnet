...
import { CriteriaEditorBase } from '../../core/widgets/_criteriaPopupEditorBase';
    ...
    constructor(options, fieldListProvider, rtl = false, _displayNameProvider, editorInputId) {
        ...
        this._disposables.push(this.popupVisible.subscribe((newVal) => {
            if (newVal) {
                ...
                this.aiFilterGenerator = this.aiFilterGenerator?.initialize(this);
...
ko.bindingHandlers['dxFilterEditor'] = {
    init: function (element, valueAccessor, bindings, viewModel) {
        ...
        const templateHtml = getTemplate(values.templateName || 'dx-filtereditor');
        ...
        if (values.aiExpressionGenerator) {
            editor.aiFilterGenerator = values.aiExpressionGenerator.initialize(editor);
        }
        if (values.popupVisible) {
            editor.popupVisible(values.popupVisible());
            computedFunctions.push(editor.popupVisible.subscribe((newVal) => {
                values.popupVisible(newVal);
            }));
            computedFunctions.push(values.popupVisible.subscribe((newVal) => {
                editor.popupVisible(newVal);
            }));
        }
