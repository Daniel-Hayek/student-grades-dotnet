...
export function wrapTreeListOptionsWithKo(options) {
    ...
    const itemsProvider = options.itemsProvider;
    if (!koUtils.isSubscribable(itemsProvider) && itemsProvider?.getItems && !itemsProvider.subscribeOnItemsChanged) {
        itemsProvider.subscribeOnItemsChanged = (item, callback) => {
            return wrapWithComputed(callback, () => itemsProvider.getItems(new PathRequest(item.path, item.pathParts)));
        };
    }
    return options;
}
