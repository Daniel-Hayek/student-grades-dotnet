...
export class AccordionKeyboardHelper extends AccessibilityKeyboardHelperBase {
    constructor(_editorViewModelsAccessor, _onToggleCollapsed = () => void 0, editorId = '') {
        super();
...
export class PropertyGridKeyboardHelper extends AccessibilityKeyboardHelperBase {
    ...
    initialize() {
        Object.keys(this._complexEditorMap)
            ...
            .forEach(headerId => {
            delete this._complexEditorMap[headerId];
            this._disposables = this._disposables.filter(d => {
                if (d instanceof AccordionKeyboardHelper && d.editorId === headerId) {
                    d.dispose();
                    return false;
                }
                return true;
        ...
        Object.keys(this._complexEditorMap).forEach(headerId => {
            if (!this._complexEditorMap[headerId].isInitialized) {
                ...
                if (elt) {
                    const helper = new AccordionKeyboardHelper(() => this._complexEditorMap[headerId].editor, () => this._defferedInit(), headerId);
