...
export class PaddingModel extends BaseRenderingMultiplatformModel {
    ...
    applyFromString(value) {
        if (value) {
            ...
            this._set('left', parseFloat(components[0]) || 0);
            this._set('right', parseFloat(components[1]) || 0);
            this._set('top', parseFloat(components[2]) || 0);
            this._set('bottom', parseFloat(components[3]) || 0);
        }
        return this;
    }
    static from(val) {
    ...
    _toString(inner = false) {
        return ['left', 'right', 'top', 'bottom'].map(x => parseFloat(this._get(x))).concat(this._get('dpi')).join(', ');
    }
