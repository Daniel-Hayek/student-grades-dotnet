...
export class SnapLinesHelper {
    ...
    getSnappingMode() {
        return this._surfaceContext().snappingMode?.();
    }
    roundToGridSize(value) {
        const snapGridSize = this._surfaceContext().snapGridSize();
        return Math.round(value / snapGridSize) * snapGridSize;
    }
    snapToGrid(uiElement, dropTarget) {
        const dropTargetY = dropTarget?.absolutePosition.y() || 0;
        const adjustToTopRespectBandsHeight = (uiElementTop) => {
            const top = typeof uiElementTop === 'string' ? convertFromCssPixelUnits(uiElementTop) : uiElementTop;
            return this.roundToGridSize(top - dropTargetY) + dropTargetY;
        };
        const adjustedLeft = this.roundToGridSize(convertFromCssPixelUnits(uiElement.dataset.leftPosition));
        const adjustedTop = adjustToTopRespectBandsHeight(uiElement.dataset.topPosition);
        uiElement.dataset.leftPosition = convertToCssPixelUnits(adjustedLeft);
        uiElement.dataset.topPosition = convertToCssPixelUnits(adjustedTop);
        ...
        if (uiElement.dataset.startDragWithOffset || uiElement.classList.contains('dx-treelist-selectedcontent')) {
            const rootElement = uiElement.closest('.dx-designer');
            const reportSurfaceRect = rootElement.querySelector('.dxrd-bands-content-offset').getBoundingClientRect();
            const designerRect = rootElement.getBoundingClientRect();
            const toolboxRect = rootElement.querySelector('.dxrd-toolbox-wrapper').getBoundingClientRect();
            const offsets = {
                vertical: reportSurfaceRect.top - designerRect.top,
                horizontal: reportSurfaceRect.left - toolboxRect.left
            };
            const ghostTop = convertFromCssPixelUnits(uiElement.ghostContainer.style.top) - offsets.vertical;
            const ghostLeft = convertFromCssPixelUnits(uiElement.ghostContainer.style.left) - offsets.horizontal;
            const roundedTop = adjustToTopRespectBandsHeight(ghostTop);
            const startDragAdjustedTop = roundedTop + offsets.vertical;
            const roundedLeft = this.roundToGridSize(ghostLeft);
            const startDragAdjustedLeft = roundedLeft + offsets.horizontal;
            uiElement.dataset.leftPosition = convertToCssPixelUnits(startDragAdjustedLeft);
            uiElement.dataset.topPosition = convertToCssPixelUnits(startDragAdjustedTop);
            uiElement.ghostContainer.style.left = uiElement.dataset.leftPosition;
            uiElement.ghostContainer.style.top = uiElement.dataset.topPosition;
        }
    }
