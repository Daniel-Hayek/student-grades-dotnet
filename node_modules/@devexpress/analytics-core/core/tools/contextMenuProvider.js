...
export class ContextMenuProvider extends Disposable {
    constructor({ actions, target, getClickActionParams, contextMenusEnabled, fullScreen }) {
        ...
        const updateOverlayContainer = (isFullScreen) => {
            this.overlayContainerClass = '.dx-designer-viewport';
            if (!isFullScreen && this.getPopupContainer('.dx-blazor-reporting')) {
                this.overlayContainerClass = '.dx-blazor-reporting';
            }
            if (this.component?._overlay?._positionController) {
                this.component.option('overlayContainer', this.getPopupContainer(this.overlayContainerClass));
            }
        };
        if (fullScreen) {
        ...
        this.onOptionChanged = (e) => {
            if (e.name === 'visible' && e.value)
                if (this.component?._overlay?._positionController?.$container) {
                    const container = this.getPopupContainer(this.overlayContainerClass);
                    const controller = this.component._overlay._positionController;
                    controller.updateContainer(container);
                    this.component.option('overlayContainer', container);
                }
        };
    ...
    getPopupContainer(containerSelector) {
        return this._menuElement?.closest(containerSelector);
    }
}
