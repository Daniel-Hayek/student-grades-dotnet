    ...
    init: function (element, valueAccessor, allBindings, viewModel, bindingContext) {
        ...
        const options = extend(initializeBaseResizableOptions(valueAccessor()), {
            ...
            resize: (event, element, boundsDiff) => {
                ...
                const snappingMode = values?.snapHelper?.getSnappingMode();
                ...
                let diffForDimensionNotations = boundsDiff;
                if (event.altKey || snappingMode !== 'SnapLines') {
                    values.snapHelper?.deactivateSnapLines();
                }
                    ...
                    if (!boundsAreEqual(snapBounds, resizedBounds)) {
                        ...
                        diffForDimensionNotations =
                            {
                                width: snapBounds.width - mainSelectionOriginalSize.width,
                                height: snapBounds.height - mainSelectionOriginalSize.height,
                            };
                        values?.resizing?.(diffForDimensionNotations);
                        return;
                    }
                }
                values?.resizing?.(diffForDimensionNotations);
        ...
        const disposeFunction = initializeResize(element, options, viewModel);
