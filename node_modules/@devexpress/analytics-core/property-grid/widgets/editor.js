...
export class Editor extends BaseRenderingMultiplatformModel {
    ...
    constructor(modelPropertyInfo, level, parentDisabled, textToSearch = undefined, popupService, popover, engineType = 'multiplatform') {
        ...
        this.addDisposable(this.createComputedProperty('disabled', () => {
            ...
            return parent?._get('disabled') || _accessibilityProvider && _accessibilityProvider.isPropertyDisabled(this);
        }, [
            subscribableProperty(this, [
    ...
    _checkVisibility() {
        const _accessibilityProvider = this._get('_accessibilityProvider');
        return this._get('isSearchedProperty') && _accessibilityProvider?.isPropertyVisible(this);
    }
    ...
    _roundTwoDecimalsForUnitProperties(value, model, propertyName) {
    ...
    _init(editorTemplate, name) {
        ...
        this.addDisposable(this.createComputedProperty('value', {
            read: () => {
                ...
                if (modelValue && typeof modelValue !== 'object' && !modelValue['push']) {
                    ...
                    this._cachedValue = hasValueInModel ? this._roundTwoDecimalsForUnitProperties(unwrappedValue, model, name) : undefined;
                    return this._cachedValue;
                }
                else {
        ...
        this.name = name;
        this.editorTemplate = editorTemplate?.custom || 'dx-property-editor';
    }
    _getInfoFromModel(viewModel) {
        if (!viewModel)
            return null;
        const modelInfo = viewModel['getInfo']?.();
        if (modelInfo) {
    ...
    _getModelSubscribableProperties(status) {
        return [
            subscribableProperty(this, [{
                    ...
                    computedProperties: () => {
                        const model = this._get('_model');
                        return model?.getPropertyDependencies?.(this.name, status) || [];
                    },
                }])
        ];
    }
