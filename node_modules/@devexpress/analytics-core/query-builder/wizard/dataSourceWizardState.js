...
const _defaultRestoreMongoDBDataSourceFromState = (state, requestWrapper, dataSourceId) => {
    ...
    if (state.queries && state.queries.length > 0) {
        dataSource.queries.push(...state.queries.map((query) => new MongoDBQuery(JSON.parse(query), new ModelSerializer())));
    }
    dataSource.name(state.dataSourceName || '');
    dataSource.id = dataSourceId || guid().replace(/-/g, '');
    return dataSource;
};
export let _restoreMongoDBDataSourceFromState = _defaultRestoreMongoDBDataSourceFromState;
export function _setRestoreMongoDBDataSourceFromState(func) { _restoreMongoDBDataSourceFromState = func; }
export function _resetRestoreMongoDBDataSourceFromState() { _restoreMongoDBDataSourceFromState = _defaultRestoreMongoDBDataSourceFromState; }
...
export function _createDefaultDataSourceWizardState(sqlDataSourceWizardState = {}, jsonDataSourceWizardState = { jsonSource: '' }, objectDataSourceWizardState = {}, federationDataSourceWizardState = {}, mongoDBDataSourceWizardState = {}) {
    return {
