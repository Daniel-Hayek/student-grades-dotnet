...
import { getParentContainer, guid, NotifyAboutWarning } from '@devexpress/analytics-core/analytics-internal';
...
export class XRRichEditControlModel extends Disposable {
    ...
    executeCommand(commandId, parameter, setFocus = false) {
        if (!this._richEdit.isDisposed) {
            if (commandId === getRichEditInstance().InsertTabCommandId.ShowHyperlinkDialog) {
                const getDialog = this._richEdit['_native'].dialogManager.getDialog.bind(this._richEdit['_native'].dialogManager);
                const container = getParentContainer(this._element);
                this._richEdit['_native'].dialogManager.getDialog = (name) => {
                    const dialog = getDialog(name);
                    if (name === 'Hyperlink' && dialog) {
                        dialog.afterShowing = () => dialog.popupDialog?.option('container', container);
                    }
                    return dialog;
                };
            }
