...
import { keepTogether } from '../controls/metadata/properties/metadata';
import { BandsHolder } from './_bandHolder';
import { insertBand, sortBands, _getUnitAbsoluteRect } from './_bandUtils';
...
export class BandViewModel extends XRReportElementViewModel {
    ...
    createChildsArray(band, serializer) {
        ...
        if (band.SubBands) {
            Object.keys(band.SubBands).forEach((key) => {
                const subBand = this.getControlFactory().createControl({
                    '@ControlType': 'SubBand',
                    ...band.SubBands[key],
                }, this, serializer);
                this.bands.push(subBand);
            });
    ...
    isPropertyVisible(name) {
        if (name === printAcrossBands.propertyName) {
            return this._isHeaderBandType(this);
        }
        else if (name === pageBreak.propertyName || name === keepTogether.propertyName) {
            return !this._isHeaderOrFooterBand(this);
        }
        else {
...
BandViewModel.unitProperties = ['height', 'paddingObj'];
export class BandSurface extends SurfaceElementBase {
    ...
    constructor(band, context, unitProperties = BandSurface._unitProperties) {
        ...
        const coordGridWidthCalc = () => {
            let width = 0;
            return () => {
                if (!checkModelReady(root)) {
                    return width;
                }
                width = root.pageWidthF() - nearMarginWidth();
                return width;
            };
        };
        this.coordinateGridOptions = {
            ...
            width: ko.pureComputed(coordGridWidthCalc()),
            flip: context.rtl
        };
        let oldDelta = 0;
        this['resize'] = (params) => {
