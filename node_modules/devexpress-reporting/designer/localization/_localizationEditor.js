...
export class LocalizationEditor extends Disposable {
    ...
    applyLocalization(serviceName) {
        if (this.language() !== defaultCulture) {
            ...
            _translationFactory.translate(serviceName, textCollection.map(x => x()), this.language(), this.currentLanguageText()).done((result) => {
    ...
    getRegisteredService() {
        const serviceName = _translationFactory.getFirstRegistered();
        if (!serviceName)
            return;
        const service = _translationFactory.getService(serviceName);
        return {
            applyLocalization: () => this.applyLocalization(serviceName),
            isDisabled: () => this.isDefaultLanguage(),
            iconClass: service._iconClass || 'apply-localization-service',
            icon: service._icon || 'dxrd-svg-actions-apply_localization_service',
        };
    }
    ...
    getLoadPanelOptions(element) {
        const position = this.getLoadPanelPosition(element);
        return {
            visible: this.showLoadIndicator,
            shading: true,
