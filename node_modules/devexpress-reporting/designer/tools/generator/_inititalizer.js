...
import { AIExpressionGenerator, editorTemplates, ObjectProperties, unwrapEditor } from '@devexpress/analytics-core/analytics-widgets';
...
import { controlsFactory, DataBindingMode as UtilsDataBindingMode, DefaultCrossTabControl, formatStringEditorCustomSet, HandlerUri as UtilsHandlerUri, smartTagFactory as UtilsSmartTagFactory, HandlerUri } from '../../utils/settings';
...
import { _registerAITranslationService, _unregisterAITranslationService } from '../../localization/localizationService';
import { AIDesignerPanelModel } from '../../internal/ai/_aiDesignerPanelModel';
import { AIServicesEnabled } from '../../../designer/settings';
import { AITabInfo } from '../../../viewer/ai/_aiPanelModel';
import { ReportPreviewServiceHelper } from '../../services/_aiService';
export class ReportDesignerInitializer extends CommonDesignerGenerator {
    ...
    addAIFeatures(aiSettings) {
        const aiServiceDisposer = AIServicesEnabled.subscribe(aiServicesEnabled => {
            if (aiServicesEnabled) {
                if (aiSettings?.enableExpressionGeneration && !this.getModel().aiExpressionGenerator)
                    this.addElement('aiExpressionGenerator', () => new AIExpressionGenerator(this.getModel()?.model, HandlerUri(), (errorMessage) => ReportStorageWeb.getErrorMessageHandler(errorMessage)(errorMessage, undefined, undefined)));
                if (aiSettings?.enableLocalization)
                    _registerAITranslationService();
            }
            else {
                this.addElement('aiExpressionGenerator', () => undefined);
                _unregisterAITranslationService();
            }
        });
        this._addDisposable({ dispose: () => {
                this.getModel?.()?.aiExpressionGenerator?.dispose();
                aiServiceDisposer?.();
            } });
        AIServicesEnabled(!!aiSettings);
        return this;
    }
    ...
    initializeFieldListActionProviders(func = () => {
        ...
        if (this.options.data?.dataSourceSettings?.allowEditDataSource ?? true) {
            ...
            if (this._mongoDBDataSourceEditor)
                providers.push(this._mongoDBDataSourceEditor);
        }
        return providers;
    }) {
    ...
    _createAITab(aiSettings) {
        const aiPanel = new AIDesignerPanelModel(this.buildingModel, aiSettings);
        this._addDisposable(aiPanel);
        const getVisible = () => {
            return aiSettings?.testDataSourceSettings?.enableTestDataSourceGeneration
                && !!this.buildingModel.model() && AIServicesEnabled();
        };
        const tabInfo = new AITabInfo(aiPanel, 'dxrd-aipanelwrapper', getVisible());
        this._addDisposable(this.buildingModel.model.subscribe(() => {
            tabInfo.visible = getVisible();
        }));
        return tabInfo;
        ...
        this.addElement('showPreview', () => () => {
            ...
            if (!this.buildingModel.model()) {
                const subscription = this.buildingModel.model.subscribe((newVal) => {
                    ...
                    reportPreview.initialize(this._previewServiceHelper.initializePreview(this.buildingModel.model()));
                });
            }
            else {
                reportPreview.initialize(this._previewServiceHelper.initializePreview(this.buildingModel.model()));
    ...
    _createNewReportFromWizardState(reportWizard, designerModel, wizardState, state, deferred) {
        ReportWizardService.generateReportFromWizardState(wizardState, reportWizard._requestModelType, state(), (wizardModel) => {
            ...
            .always(() => { designerModel.isLoading(false); });
    }
        ...
        return ((actions) => {
            ...
            if (reportPreview) {
                ...
                const previewAction = new PreviewAction(reportPreview, {
                    ...
                    isContextMenuAction: false,
                    clickAction: () => {
                        reportPreview._show(this._previewServiceHelper.initializePreview(report()));
                    },
                    hasSeparator: true
                });
                ...
                const testDataSourceSettings = this.options.aiSettings?.testDataSourceSettings;
                if (testDataSourceSettings?.enableTestDataSourceGeneration && AIServicesEnabled()) {
                    const aiPreviewAction = new BaseAction({
                        text: getLocalization('Test Data', 'AIReportStringId.ReportTestData_Button'),
                        displayText: () => getLocalization('Preview the report document with AI-generated data.', 'AIReportStringId.ReportTestData_ButtonToolTip'),
                        imageClassName: 'dxrd-image-ai-preview',
                        imageTemplateName: 'dxrd-svg-operations-ai',
                        templateName: reportPreview.canSwitchToDesigner ? 'dxrd-toolbar-two-way-switch' : undefined,
                        disabled: ko.pureComputed(() => { return !report(); }),
                        clickAction: () => {
                            const reportPreview = this.buildingModel.reportPreviewModel.reportPreview;
                            const report = this.buildingModel.model();
                            reportPreview._show(this._previewServiceHelper.initializePreview(report, true), true);
                        }
                    });
                    previewAction.contentData.items.push({ itemData: aiPreviewAction, active: false });
                    const aiEnabledSubscription = AIServicesEnabled.subscribe(aiServicesEnabled => {
                        aiPreviewAction.visible = aiServicesEnabled;
                    });
                    this._addDisposable({ dispose: () => aiEnabledSubscription?.() });
                }
    ...
    tryAddCollectionDataSourceFilterEditorAddon(filterEditor = this._dataSourceActionProvider.filterEditor) {
        if (filterEditor)
            this.buildingModel.addOns.push({ id: ReportDesignerAddOns.FilterEditor, templateName: ReportDesignerAddOns.FilterEditor, model: filterEditor });
        return this;
    }
    ...
    addWizards(connectionStrings, wizardSettings, customizeSettingsFunc) {
        ...
        this._mongoDBDataSourceEditor = initializerSettings.mongoDBDataSourceEditor;
    ...
    constructor(options) {
        ...
        this._previewServiceHelper = new ReportPreviewServiceHelper();
