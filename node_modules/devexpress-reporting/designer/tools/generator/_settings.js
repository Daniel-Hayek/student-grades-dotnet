...
import { MongoDBDataSourceEditor } from '../../actions/_mongoDBDataSourceEditor';
export class WizardsInitializerSettings {
    _doFinishCallback(data, connections) {
        ...
        else if (data.dataSourceType === DataSourceType.MongoDB) {
            const deferred = new DxDeferred();
            this.mongoDBDataSourceEditor.applyDataSourceWizardChanges(data)
                .done((result) => { deferred.resolve(result); })
                .fail(() => deferred.reject());
        }
    ...
    createSqlDataSourceEditor(settings) {
        ...
        this.mongoDBDataSourceEditor = new MongoDBDataSourceEditor(settings.dataSourceHelper, settings.dataSourceWizard, settings.model, settings.undoEngine, settings.fieldListProvider);
        ...
        this.dataSourceActionProvider = new DataSourceActions(settings.dataSourceHelper, settings.model, settings.undoEngine, settings.allowEditDataSource, settings.allowRemoveDataSource, settings.fieldListProvider, settings.displayNameProvider);
    }
    ...
    createMultiQueryDataSourceWizard(disableCustomSql, multipleQueriesWizardCallbacks = this.multipleQueriesWizardCallbacks, allowCreateNewJsonConnection = false) {
        ...
        this.multiQueryWizardOptions.getMongoDBConnectionStrings = () => ReportDataSourceService.getWizardMongoDBDataConnections();
    ...
    createReportWizard(settings) {
        this.reportWizardOptions.aiReportGenerationSettings = settings.data.aiSettings?.reportGenerationSettings;
        ...
        this.reportWizardOptions.dataSources = ko.pureComputed(() => {
            ...
            return result.filter(object => !object.hasErrors);
        });
        this.reportWizardOptions.callbacks = extend({}, this.multipleQueriesWizardCallbacks, {
            ...
            createMongoDBDataSourceInfo: MongoDBDataSourceEditor.createMongoDBDataSourceInfo,
            ...
            finishCallback: (reportWizardModel) => {
                ...
                ReportWizardService.generateReportFromWizardState(reportWizardModel, MasterDetailRequestModel, settings.state(), (wizardModel) => {
                    ...
                    .fail(() => { deferred.reject(); })
                    .always(() => { settings.isLoading?.(false); });
                return deferred.promise();
            }
        });
