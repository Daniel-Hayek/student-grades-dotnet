...
export class ReportActions extends Disposable {
    _canAddBand(bandType) {
        ...
        if (bandType === 'SubBand') {
            ...
            return model !== null && !model.lockedInUserDesigner() && (model.controlType.indexOf('XtraReport') === -1 && subBandParentBlacklist.indexOf(model.controlType) === -1);
        }
    ...
    _addBand(bandType) {
        if (this._canAddBand(bandType)) {
            ...
            const child = { '@ControlType': bandType };
            if (!['VerticalHeaderBand', 'VerticalDetailBand', 'VerticalTotalBand'].includes(bandType)) {
                const dpi = model.root['dpi']();
                child['@HeightF'] = dpi;
                child['@Dpi'] = dpi;
            }
            const control = model.createChild(child);
