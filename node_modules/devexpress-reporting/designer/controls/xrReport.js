...
*/
import { Margins, SurfaceElementArea } from '@devexpress/analytics-core/analytics-elements';
...
import { getLevelCount, REMOVED_DETAILBAND_LEVEL, setMarkerWidth, sortBands } from '../bands/_bandUtils';
...
import { pageHeightF, pageWidthF } from './metadata/xrReport';
...
import { DetailBandSurface } from '../bands/xrDetailBand';
...
export class ReportViewModel extends XRReportElementViewModel {
    static createObjectStorage(_componentStorage, _objectStorage, collectSubscription = (subscription) => void 0) {
        ...
        collectSubscription(objectStorage.subscribe((changeSet) => {
            changeSet.forEach((change) => {
                const objectType = ko.unwrap(change.value.objectType);
                if (change.status === 'added') {
                    if (objectType && ReportViewModel.availableDataSourceTypes.some(x => objectType.indexOf(x) !== -1)) {
                    ...
                    if (objectType && ReportViewModel.availableDataSourceTypes.some(x => objectType.indexOf(x) !== -1) && _componentStorage().indexOf(change.value) !== -1) {
    ...
    _updatePageSize(paperKind, prevDpi) {
        const size = paperKind === 'Custom' ? this.size : getPaperSize(paperKind);
        const oldDpi = paperKind === 'Custom' ? prevDpi : 100;
        const precision = this.measureUnit() === 'Inches' || this.measureUnit() == 'Millimeters' ? 2 : 0;
        this.pageHeightF(roundingXDecimals(ko.unwrap(this.landscape() ? size.width : size.height) * (this._innerDpi.peek() / oldDpi), false, precision));
        this.pageWidthF(roundingXDecimals(ko.unwrap(this.landscape() ? size.height : size.width) * (this._innerDpi.peek() / oldDpi), false, precision));
    }
    ...
    isPropertyDisabled(name) {
        if (name === 'pageWidthF' || name === 'pageHeightF') {
    ...
    beforeDeserialize(reportModel) {
        this.controlType = 'DevExpress.XtraReports.UI.XtraReport';
        if (reportModel['@PageWidth'] !== undefined) {
            reportModel['@PageWidthF'] = reportModel['@PageWidth'];
            delete reportModel['@PageWidth'];
        }
        if (reportModel['@PageHeight'] !== undefined) {
            reportModel['@PageHeightF'] = reportModel['@PageHeight'];
            delete reportModel['@PageHeight'];
        }
    }
    constructor(report, serializer, knownEnums) {
        ...
        if (this.pageWidthF() === pageWidthF.defaultVal) {
            this.pageWidthF(ReportViewModel.defaultPageSize.width);
        }
        if (this.pageHeightF() === pageHeightF.defaultVal) {
            this.pageHeightF(ReportViewModel.defaultPageSize.height);
        }
        ...
        this.displayNameProvider = ko.observable();
        ...
        this._disposables.push(this.measureUnit.subscribe((unitType) => {
            this._update(true);
            const prevDpi = this._innerDpi();
            this._recalculateUnits(unitType);
            this._updatePageSize(this.paperKind.peek(), prevDpi);
            this._update(false);
        }));
        this._disposables.push(this.landscape.subscribe((newVal) => {
            const width = this.pageWidthF();
            this.pageWidthF(this.pageHeightF());
            this.pageHeightF(width);
        }));
        ...
        this._disposables.push(this.margins.left.subscribe((newVal) => {
            if (this.isModelReady() && newVal > this.pageWidthF() - this.margins.right()) {
                this.margins.right(this.pageWidthF() - this.margins.left());
            }
        }));
        this._disposables.push(this.margins.right.subscribe((newVal) => {
            if (this.isModelReady() && newVal > this.pageWidthF() - this.margins.left()) {
                this.margins.left(this.pageWidthF() - this.margins.right());
            }
        }));
        this._disposables.push(this.paperKind.subscribe((newVal) => {
            newVal !== 'Custom' && this._updatePageSize(newVal, this._innerDpi());
        }));
    ...
    shouldSerialize(propertyName, viewModel) {
        if (propertyName === 'margins') {
            return !(this.measureUnit() === 'HundredthsOfAnInch' && viewModel.margins.toString() === Margins.defaultVal);
        }
        return true;
    }
    isLocalized() {
    ...
    initialize() {
        super.initialize();
        this.size.height = this.pageHeightF;
        this.size.width = this.pageWidthF;
    }
    ...
    addChild(control) {
        if (control instanceof XRCrossBandControlViewModel) {
            ...
            if (control instanceof DetailReportBand && control.level() === REMOVED_DETAILBAND_LEVEL) {
                control.level(0);
            }
        }
        else {
    ...
    removeChild(control) {
        if (control instanceof XRCrossBandControlViewModel) {
            ...
            if (control instanceof DetailReportBand)
    ...
    serialize() {
        ...
        const serializedReport = new ReportModelSerializer(this).serialize();
        this._localizationItems.removeAll();
...
ReportViewModel.unitProperties = ['snapGridSize', 'margins', 'paddingObj'];
    ...
    constructor(report, zoom = ko.observable(1)) {
        ...
        this._assignSelectedWatermark = () => {
            ...
            this._watermarkSubscriptions.push(this._watermarkTextRenderingResult = ko.pureComputed(() => {
                ...
                const originalWidthPx = unitsToPixel(this.report.pageWidthF(), this.measureUnit());
                const originalHeightPx = unitsToPixel(this.report.pageHeightF(), this.measureUnit());
        ...
        this.surfaceContextMenuHandler = (selection, surface, e) => {
            ...
            if (focused instanceof XRTableSurface && selection.dropTarget instanceof XRTableCellSurface
                ...
                return;
            if ((focused instanceof DetailReportBandSurface || focused instanceof DetailBandSurface)
                && (selection.dropTarget instanceof DetailBandSurface || selection.dropTarget instanceof DetailReportBandSurface))
                return;
            !selection.disabled() && selection.clickHandler(selection.dropTarget ? null : surface, e);
        };
        this.isFit = (dropTarget) => {
        ...
        this.snappingMode = report.snappingMode;
        this._context = this;
