/**
* DevExpress HTML/JS Reporting (designer\controls\crossTab\definitions.js)
* Version:  25.2.4
...
import { crossTabColumnDefinitionInfo, crossTabRowDefinitionInfo } from '../metadata/crosstab/definitions';
        ...
        for (let i = 0; i < matrixHelper.columnCount; i++) {
            for (let j = 0; j < matrixHelper.rowCount; j++) {
                ...
                if (cell._columnSpan() === 1 && !this.findDefinition(this._columnDefinitions, cell)) {
                    this._columnDefinitions.push({ cell: cell, definition: crossTab._columnDefinitions()[i] });
                }
                if (cell._rowSpan() === 1 && !this.findDefinition(this._rowDefinitions, cell)) {
                    this._rowDefinitions.push({ cell: cell, definition: crossTab._rowDefinitions()[j] });
                }
            }
        }
    }
            ...
            for (let j = 0; j < matrixHelper.rowCount; j++) {
                ...
                if (columnDefs[i] == null && cell._columnSpan() === 1) {
                    ...
                    if (element && element.definition)
                        columnDefs[i] = new CrossTabColumnDefinitionsModel(this._serializer.serialize(element.definition));
                }
                if (rowDefs[j] == null && cell._rowSpan() === 1) {
                    const element = this.findDefinition(this._rowDefinitions, cell);
                    if (element && element.definition)
                        rowDefs[j] = new CrossTabRowDefinitionsModel(this._serializer.serialize(element.definition));
                }
            }
        }
        for (let i = 0; i < matrixHelper.columnCount; i++) {
