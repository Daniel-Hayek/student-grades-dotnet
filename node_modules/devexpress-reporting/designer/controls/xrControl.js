...
const patchCanPublish = (model) => {
    if (model['@CanPublish']) {
        model['@CanPublishOptions'] = model['@CanPublish'] === 'false' ? 'None' : 'All';
        delete model['@CanPublish'];
    }
    return model;
};
var FieldNameOptions;
(function (FieldNameOptions) {
    FieldNameOptions[FieldNameOptions["DisplayName"] = 0] = "DisplayName";
    FieldNameOptions[FieldNameOptions["RealName"] = 1] = "RealName";
...
export class XRControlViewModel extends XRReportElementViewModel {
    ...
    beforeDeserialize(model, parent, serializer) {
        patchCanPublish(model);
    }
    getFieldName(value, fieldNameOption) {
        const reportModel = this.root;
        const dnp = reportModel?.['displayNameProvider']?.();
        if (!dnp) {
            return null;
        }
        const startIndex = value.indexOf('[');
        const endIndex = value.indexOf(']');
        const dataMember = value.substring(startIndex + 1, endIndex);
        const dataSourceInfo = reportModel.dataSource?.()?.['dataSourceInfo'];
        const dataSourceRef = dataSourceInfo?.ref || dataSourceInfo?.id;
        if (dataSourceRef != null) {
            switch (fieldNameOption) {
                case FieldNameOptions.DisplayName:
                    return { dataMember, namePromise: dnp.getDisplayNameByPath(`${dataSourceRef}.${reportModel.dataMember()}`, dataMember) };
                case FieldNameOptions.RealName:
                    return { dataMember, namePromise: dnp.getRealName(`${dataSourceRef}.${reportModel.dataMember()}`, dataMember) };
            }
        }
    }
    setTextValue(fieldNamePromise, originText, replacedText, propertyToSet) {
        if (!fieldNamePromise)
            propertyToSet(originText);
        fieldNamePromise.done((result) => {
            const newValue = originText.replace(replacedText, result);
    ...
    constructor(control, parent, serializer) {
        ...
        if (this.text) {
            this._realText = this.text;
            this._textArea = this.text;
            this._textUI = ko.observable('');
            this._disposables.push(this.text = ko.computed({
                read: () => this._realText(),
                write: newTextValue => {
                    const fieldName = this.getFieldName(newTextValue, FieldNameOptions.RealName);
                    if (!fieldName) {
                        this._realText(newTextValue);
                        return;
                    }
                    this.setTextValue(fieldName.namePromise, newTextValue, fieldName.dataMember, this._realText);
                }
            }));
            this._disposables.push(this.textUI = ko.computed({
                read: () => {
                    ...
                    if (!fieldName)
                        return textValue;
                    this.setTextValue(fieldName.namePromise, textValue, fieldName.dataMember, this._textUI);
                    return this._textUI();
                },
                write: newTextValue => {
            ...
            this._disposables.push(this.textArea = ko.computed({
                read: () => this.textUI(),
                write: newTextValue => {
                    this.text(newTextValue);
                }
        ...
        if (control.Action) {
            ...
            this.action = ko.observable(new ActionTypeBase(this, this.name()));
        }
        this._disposables.push(this.action.subscribe(value => {
            if (!value) {
                this.action(new ActionTypeBase(this, this.name()));
            }
        }));
    ...
    isPropertyDisabled(name) {
        if (super.isPropertyDisabled(name)) {
            return true;
        }
        if (name === 'textFitMode') {
...
export class XRControlSurfaceBase extends SurfaceElementBase {
    ...
    constructor(control, context, unitProperties) {
        ...
        this.showDropBorders = ko.observable(false);
    }
    ...
    displayText() {
        ...
        let text = ko.unwrap(this._control['textUI']) || ko.unwrap(this._control['text']) || '';
