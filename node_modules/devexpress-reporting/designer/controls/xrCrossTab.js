...
import { CrossTabColumnDefinitionsModel, CrossTabRowDefinitionsModel, DefinitionUpdater, findcells } from './crossTab/definitions';
import { DataFieldLayout, TotalsPosition } from './crossTab/enums';
import { crossTabCellHeight, crossTabCellWidth } from './metadata/crosstab/definitions';
...
import { dpi as dpiMetadata } from './metadata/properties/metadata';
...
export class XRCrossTabViewModel extends XRControlViewModel {
    ...
    updateLayout() {
        ...
        undo && undo.start();
        this.update(true);
        this.definitionUpdater = new DefinitionUpdater(this);
        this.cellCreator = CellCreator.createInstance(this);
        const cells = this.cellCreator.create();
        const modelCells = this.applyCells(cells);
        this._cells(modelCells);
        const columnWidth = crossTabCellWidth.defaultVal / dpiMetadata.defaultVal * this.dpi();
        const rowHeight = crossTabCellHeight.defaultVal / dpiMetadata.defaultVal * this.dpi();
        const defs = this.definitionUpdater.update(modelCells, columnWidth, rowHeight);
