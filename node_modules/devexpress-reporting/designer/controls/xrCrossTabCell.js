...
export class XRCrossTabCellViewModel extends XRControlViewModel {
    ...
    get _rowVisible() {
        const rows = this.parent._rowDefinitions().slice(this._rowIndex(), this._rowIndex() + this._rowSpan());
        return !rows.every(x => !x.visible());
    }
    set _rowVisible(value) {
        this.parent.update(true);
        for (let i = 0; i < this._rowSpan(); i++) {
            this.parent._rowDefinitions()[this._rowIndex() + i].visible(value);
        }
        this.parent.update(false);
    }
    get _columnVisible() {
        const columns = this.parent._columnDefinitions().slice(this._columnIndex(), this._columnIndex() + this._columnSpan());
        return !columns.every(x => !x.visible());
    }
    set _columnVisible(value) {
        this.parent.update(true);
        for (let i = 0; i < this._columnSpan(); i++) {
            this.parent._columnDefinitions()[this._columnIndex() + i].visible(value);
        }
        this.parent.update(false);
    }
    constructor(model, parent, serializer = new ModelSerializer()) {
        ...
        this.rowVisible = ko.observable(this._rowVisible);
        this._disposables.push(this.rowVisible.subscribe((value) => {
            if (checkModelReady(this.parent)) {
        ...
        this._disposables.push(ko.computed(() => {
            if (checkModelReady(this.parent)) {
                this.rowVisible(this._rowVisible);
            }
        }));
        this.columnVisible = ko.observable(this._columnVisible);
        this._disposables.push(this.columnVisible.subscribe((value) => {
            if (checkModelReady(this.parent)) {
                this._columnVisible = value;
            }
        }));
        this._disposables.push(ko.computed(() => {
            if (checkModelReady(this.parent)) {
    ...
    isPropertyVisible(name, isPopularProperty = false) {
        ...
        if (name == 'visible' || name == 'location' || name == 'canPublishOptions')
            return false;
        if (name == 'fieldName')
