...
export class XRReportElementViewModel extends ElementViewModel {
    ...
    normalizePaddings() {
        if (!this.paddingObj)
            return;
        const surfaceDpi = ko.unwrap(this.surface)?.dpi?.() || this.parentModel?.()?.dpi?.() || 1;
        const properties = PaddingModel.unitProperties;
        properties?.forEach(property => {
            const paddingValue = ko.unwrap(this.paddingObj['_' + property] || this.paddingObj[property]);
            const normalizedPaddingValue = (paddingValue >= 1 && (paddingValue / surfaceDpi) >= 1) ? (paddingValue / 100) : paddingValue;
            this.paddingObj[property](normalizedPaddingValue);
        });
    }
    ...
    _syncDpi() {
        this._syncDpiCore(this.parentModel());
        this._disposables.push(this.parentModel.subscribe((parent) => this._syncDpiCore(parent)));
    }
    _syncDpiCore(parent) {
        if (!parent?.dpi || parent.dpi() === this._innerDpi())
            return;
        getUnitProperties(this).reCalculateObject(parent.dpi() / this._innerDpi());
        this._innerDpi(parent.dpi());
    }
    ...
    initExpressionProperties() {
        ...
        this.expressionObj = this.getControlFactory()._createExpressionObject(this, this.expressionBindings, path, (name) => this['Summary'] && this['Summary']['Running'] && this.getControlInfo().defaultBindingName === name && ko.computed(() => {
