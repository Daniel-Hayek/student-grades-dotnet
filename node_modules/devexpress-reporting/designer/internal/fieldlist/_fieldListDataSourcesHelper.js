...
import { _getPathPartsFromRequest, ModelSerializer, PathRequest } from '@devexpress/analytics-core/analytics-utils';
...
export class FieldListDataSourcesHelper {
    ...
    _subscribeDataSource(dataSource) {
        if (!dataSource.data)
            return;
        const subscription = this._dataSourceChangeSubscription(dataSource);
        if (dataSource.data.base64) {
            this._dataSourceSubscriptions.push(dataSource.data.base64.subscribe(subscription));
        }
        if (dataSource.data.objectTypeName && dataSource.data.objectTypeName() !== undefined) {
            this._dataSourceSubscriptions.push(dataSource.data.objectTypeName.subscribe(subscription));
        }
    }
    ...
    constructor() {
        ...
        this._dataSourceChangeSubscription = (dataSource) => () => {
    ...
    _getItemsFromCache(currentRequest) {
        ...
        const closestCachedPath = _getPathPartsFromRequest(currentRequest);
        while (!items && closestCachedPath.length > 1) {
            closestCachedPath.pop();
            items = this._innerCache[closestCachedPath.join('.')];
        }
        let currentPath = _getPathPartsFromRequest(currentRequest);
    ...
    _subscribeDataSources(usedDataSources, model) {
        this._usedDataSourceSubscription = usedDataSources.subscribe((args) => {
            ...
            if (changeSet.status === 'added') {
                ...
                model.components.push(new ComponentsModel(dataSource, this._renameDataSourceStrategy, model.parameters));
    ...
    updateDataSources(dsHelper, model, parameters) {
        ...
        model.components(dsHelper.usedDataSources()
            ...
            .map(item => new ComponentsModel(item, this._renameDataSourceStrategy, parameters)));
    }
}
