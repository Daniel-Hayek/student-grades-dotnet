...
import { ErrorCategory, ErrorSource, ErrorType } from './_types';
export class ErrorPanelViewModel extends Disposable {
    ...
    getTitleMessage() {
        return [this._errorMessage(), this._warningMessage(), this._informationMessage(), this._accessibilityCheckerMessage].join('\n');
    }
    assignErrors() {
                ...
                items: [
                    {
                        ...
                        data: {
                            ...
                            items: [
                                ...
                                { template: 'dxrd-error-type-filter-item', text: () => this._accessibilityCheckerMessage, icon: ErrorType.Accessibility }
    ...
    _initDefaultFilter() {
        ...
        if (this._initOptions?.showAccessibilityCategory)
            this._choosenTypes.push(ErrorType.Accessibility);
    }
    _assignFilter() {
        const filter = [];
        ...
        if (uncheckedTypes.length) {
            if (uncheckedTypes.includes(ErrorType.Accessibility)) {
                filter.push([['errorType', 'noneof', uncheckedTypes], 'and', ['errorCategory', 'noneof', [ErrorCategory.Accessibility]]]);
            }
            else {
        ...
        const filters = filter.reduce((res, val) => {
            if (!res.length)
                res.push(val);
            else
                res.push('and', val);
        ...
        this._filterValue(filters);
    }
    getIconTemplateByErrorCategory(rowData) {
        if (rowData.errorCategory === ErrorCategory.Accessibility) {
            return 'dxrd-svg-errorPanel-accessibility';
        }
        return this.getIconTemplateName(rowData.errorType);
    }
    getIconTemplateName(errorType) {
        return 'dxrd-svg-errorPanel-' + ErrorType[errorType].toLowerCase();
    }
    getTitleByErrorType(errorType) {
        return ErrorType[errorType].toLowerCase();
    }
    _getTotalCountByErrorType(errorType) {
        return this._filteredErrorList().filter(x => x.errorType === errorType &&
            (x.errorCategory === ErrorCategory.General ||
                (this._choosenTypes().includes(ErrorType.Accessibility) &&
                    x.errorCategory === ErrorCategory.Accessibility))).length;
    }
    constructor(options) {
        super();
        this._offset = 20;
        ...
        this._providers = [];
        this._subscriptions = [];
        this._accessibilityCheckerMessage = getLocalization('Accessibility', 'PreviewStringId.ReportDesignAnalyzer_Accessibility');
        ...
        })), this._errorMessage = ko.computed(() => this._createMessage(this._getTotalCountByErrorType(ErrorType.Error), getLocalization('{0} Error|{0} Errors', 'PreviewStringId.ReportDesignAnalyzer_Error'))), this._warningMessage = ko.computed(() => this._createMessage(this._getTotalCountByErrorType(ErrorType.Warning), getLocalization('{0} Warning|{0} Warnings', 'PreviewStringId.ReportDesignAnalyzer_Warning'))), this._informationMessage = ko.computed(() => this._createMessage(this._getTotalCountByErrorType(ErrorType.Information), getLocalization('{0} Message|{0} Messages', 'PreviewStringId.ReportDesignAnalyzer_Message'))));
    }
}
