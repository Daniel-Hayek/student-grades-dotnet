...
export class ExpressionWrapper extends Disposable {
    ...
    _createExpressionMap(control, propertyName, eventName, expressions, subscriptions, path, summaryRunning) {
    ...
    _mapExpressionsToObjectByEventName(control, object, eventName, expressions, subscriptions, path, summaryRunning) {
        const properties = object.getInfo();
        properties.forEach(info => {
            if (!ko.isObservable(object[info.propertyName])) {
                this._mapExpressionsToObjectByEventName(control, object[info.propertyName], eventName, expressions, subscriptions, path, summaryRunning);
            }
            else {
                object[info.propertyName] = this._createExpressionMap(control, info.propertyName, eventName, expressions, subscriptions, path, summaryRunning);
    ...
    _mapExpressionsToObject(control, expressions, path, summaryRunning) {
        ...
        const stateObj = {
            ...
            updateExpressionObjectProperties: (newExpressionProperties) => {
                ...
                if (useEvents) {
                    ...
                    properties.forEach(info => {
                        this._mapExpressionsToObjectByEventName(control, stateObj[info.propertyName], info.propertyName, expressions, subscriptions, path, summaryRunning);
                    });
                }
                else {
                    this._mapExpressionsToObjectByEventName(control, stateObj, 'BeforePrint', expressions, subscriptions, path, summaryRunning);
                }
    ...
    createExpressionsObject(control, expressions, path, summaryRunning) {
        const result = this._mapExpressionsToObject(control, expressions, path, summaryRunning);
        if (!result)
            return;
