...
import { defaultPageSetupState, ReportUnit } from '../reportWizardState';
...
export class ConfigureReportPageSettingsPage extends WizardPageBase {
    ...
    constructor() {
        ...
        this.lookupData = {
            ...
            reportUnit: [
                { value: ReportUnit.Inches, displayName: getLocalization('Inches', 'DevExpress.XtraReports.UI.ReportUnit.Inches') },
                { value: ReportUnit.HundredthsOfAnInch, displayName: getLocalization('Hundredths of an Inch', 'DevExpress.XtraReports.UI.ReportUnit.HundredthsOfAnInch') },
                { value: ReportUnit.Millimeters, displayName: getLocalization('Millimeters', 'DevExpress.XtraReports.UI.ReportUnit.Millimeters') },
                { value: ReportUnit.TenthsOfAMillimeter, displayName: getLocalization('Tenths of a Millimeter', 'DevExpress.XtraReports.UI.ReportUnit.TenthsOfAMillimeter') },
                { value: ReportUnit.Pixels, displayName: getLocalization('Pixels', 'DevExpress.XtraReports.UI.ReportUnit.Pixels') }
            ]
        };
        this.paperKind = ko.observable(defaultPageSetupState.paperKind);
        this.landscape = ko.observable(defaultPageSetupState.landscape);
        this.width = ko.observable(defaultPageSetupState.width).extend({ deferred: true });
        this.height = ko.observable(defaultPageSetupState.height).extend({ deferred: true });
        this._disposables.push(this.fixedSize = ko.computed(() => this.paperKind() !== 'Custom'));
        this.marginTop = ko.observable(defaultPageSetupState.marginTop).extend({ deferred: true });
        this.marginRight = ko.observable(defaultPageSetupState.marginRight).extend({ deferred: true });
        this.marginBottom = ko.observable(defaultPageSetupState.marginBottom).extend({ deferred: true });
        this.marginLeft = ko.observable(defaultPageSetupState.marginLeft).extend({ deferred: true });
        ...
        this._reportUnit = ko.observable(defaultPageSetupState.reportUnit);
        this._disposables.push(this.reportUnit = ko.computed({
            read: () => this._reportUnit(),
            write: (newVal) => {
                const converter = PageSetupHelper.getConverter(this._reportUnit(), newVal);
                [this.width, this.height, this.marginTop, this.marginRight, this.marginBottom, this.marginLeft].forEach(x => { x(converter(x())); });
                this._reportUnit(newVal);
            }
        }));
        this._disposables.push(this.paperKind.subscribe(newVal => {
            if (newVal !== 'Custom') {
                const convert = PageSetupHelper.getConverter(ReportUnit.Inches, this._reportUnit());
    ...
    initialize(state) {
        ...
        this.reportUnit(state.reportUnit);
...
export function _applyPageSetting(data, state) {
    ...
    state.reportUnit = data.reportUnit;
