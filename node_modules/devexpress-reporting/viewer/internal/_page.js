...
import { $dx, DxDeferred, formatUnicorn, getLocalization, toQueryParameters, base64DecodeUnicode, guid } from '@devexpress/analytics-core/analytics-internal-native';
...
export class PreviewPage extends BaseRenderingModel {
    ...
    onPropertyChanged(args) {
        ...
        if (args.propertyName === 'imageSrc')
            this._onImageSrcChanged && this._onImageSrcChanged();
    }
    ...
    _onPageLoaded(result, processClick) {
        if (!result)
            return;
        if (this._useOriginal) {
            this._useOriginal = false;
            this.aiCustomDocumentStorageId = undefined;
        }
    ...
    constructor(reportPreview, pageIndex, processClick, subscribeToPageLoading = false) {
        ...
        this.page_id = 'page' + '_' + this.pageIndex;
    ...
    getActiveBrick() {
        ...
        getActiveBricks(this.brick, activeBricks);
        return activeBricks;
    }
    clearBricks() {
    ...
    dispose() {
        ...
        this.bricks = [];
        this.events?.dispose();
        this.smartTagAIMenu = null;
        this.editingFields?.forEach(field => field.dispose && field.dispose());
        this.editingFields = [];
        this.clickableBricks?.['_viewModelEvents']?.dispose();
        this.editingFieldsKeyboardHelper?.dispose();
        this.editingFieldsKeyboardHelper = null;
    }
    revertToOriginal() {
    ...
    _updateImageSrc(includeBricks) {
        this.imageSrc = HandlerUri() + '?' + toQueryParameters({
            actionKey: 'getPage',
            unifier: this.reportPreview._unifier,
            arg: JSON.stringify(this._getCurrentPageRequest(this.reportPreview.documentId, !includeBricks))
        });
    }
    _getCurrentPageRequest(documentId, shouldSkipBricks) {
        const includeBricks = shouldSkipBricks === undefined ? this.brickLoading : !shouldSkipBricks;
        return {
            useSvgEngine: this._useSvg,
            pageIndex: this.pageIndex,
            documentId: documentId,
            resolution: Math.floor(this.actualResolution * this._getPixelRatio()),
            includeBricks: includeBricks,
            customBrickDataStorageId: this.aiCustomDocumentStorageId,
            useOriginal: this._useOriginal,
            nonce: guid()
        };
    }
    ...
    initializeBrick(brick, processClick, editingFieldBricks) {
        const currentActiveBricksMap = this.activeBricks?.length ? this.activeBricks.reduce((map, brick) => {
            map[brick.indexes] = brick;
            return map;
        }, {}) : null;
        initializeBrick(brick, processClick, editingFieldBricks, currentActiveBricksMap);
...
__decorate([
    mutable('')
], PreviewPage.prototype, "aiCustomDocumentStorageId", void 0);
__decorate([
    mutable(false)
...
__decorate([
    mutable('')
], PreviewPage.prototype, "page_id", void 0);
__decorate([
    mutable(false)
], PreviewPage.prototype, "loading", void 0);
__decorate([
    mutable({})
