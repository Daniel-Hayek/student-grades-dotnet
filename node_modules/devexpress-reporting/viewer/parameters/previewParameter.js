...
export class PreviewParameter extends BaseModel {
    ...
    constructor(parameterInfo, parameterHelper) {
        ...
        this._originalLookUpValues = parameterInfo.LookUpValues ? parameterHelper.mapLookUpValues(this.type, parameterInfo.LookUpValues || [], parameterInfo.AllowNull) : null;
    ...
    safeAssignObservable(name, value, forceUpdateValue = false) {
        if (name in this) {
    ...
    initialize(value, parameterHelper) {
        let resultValue;
        let forceUpdateValue = false;
        if (this.isMultiValueWithLookUp) {
            forceUpdateValue = !compareArrays(this?.value?.value, value?.value);
            this.safeAssignObservable('_value', (value || []).map((arrayItem) => {
                return parameterHelper.getValueConverter(this.type, this.allowNull)(arrayItem);
            }));
            const multiValuesHelper = new MultiValuesHelper(this);
            this._disposables.push(multiValuesHelper);
            let newItems;
            ...
            const converter = parameterHelper.getValueConverter(this.type, this.allowNull);
            resultValue = [this._originalValue.Start, this._originalValue.End].map(x => converter(x));
        }
        else {
            resultValue = parameterHelper.getValueConverter(this.type, this.allowNull)(value);
        }
        this.safeAssignObservable('value', resultValue, forceUpdateValue);
    }
    serialize() {
    ...
    dispose() {
        super.dispose();
        this.valueInfo = null;
        this.multiValueInfo = null;
        this.lookUpValues = null;
        this._originalLookUpValues = null;
    }
}
__decorate([
    mutable(false)
