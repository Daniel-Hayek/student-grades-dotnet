...
export class ParametersPanelModelBase extends ParameterPanelItemBase {
    ...
    subscribeParameter(parameter) {
        ...
        if (this._shouldProcessParameter(parameter)) {
            parameter.addDisposable(parameter.events.on(`${parameter.isMultiValueWithLookUp ? '_value' : 'value'}Changed`, (newValue) => {
    ...
    _setLookUpValues(parameter, lookUpValues) {
        ...
        parameter.lookUpValues = this.parameterHelper.mapLookUpValues(parameter.type, lookUpValues, parameter.allowNull);
    ...
    _getParameterValuesContainedInLookups(parameterLookUpValues, parameter) {
        if (parameterLookUpValues) {
            if (parameter.isMultiValue) {
                ...
                return selectedItems.filter((item) => this._filterParameterValuesContainsInLookups(parameterLookUpValues, parameter.type, parameter.allowNull, item).length > 0);
            }
            else {
                return this._filterParameterValuesContainsInLookups(parameterLookUpValues, parameter.type, parameter.allowNull, parameter.value);
            }
        }
        return [];
    }
    _filterParameterValuesContainsInLookups(parameterLookUpValues, parameterType, allowNull, value) {
        return parameterLookUpValues.filter((x) => PreviewParameter._compareValues(this.parameterHelper.getValueConverter(parameterType, allowNull)(x.Value), value));
    }
    ...
    constructor(parametersHolder, parameterHelper, enableKeyboardSupport, getRootElement) {
        ...
        this._disposables.push({
            dispose: () => {
                ...
                this.lastValidationRequest?.reject();
            }
        });
        this.buttons = [{
    ...
    initialize(originalParametersInfo) {
        ...
        this._parameters.forEach(x => {
            ...
            x.addDisposable(nativeMultiPlatformEngine.createComputedProperty(model, 'value', {
                read: () => {
            ...
            x.addDisposable(nativeMultiPlatformEngine.createComputedProperty(model, 'selectAll', {
                read: () => {
                    const value = x.value;
            ...
            model['parameterDescriptor'] = x.getParameterDescriptor(),
                x.addDisposable(nativeMultiPlatformEngine.createComputedProperty(model, 'lookUpValues', {
                    read: () => x.lookUpValues,
    ...
    _validateAndSubmit(params) {
        if (this.lastValidationRequest?._state === 'pending')
            return;
        const result = params?.validationGroup?.validate?.();
        if (result && result.status === 'pending' && result.complete.then) {
            this.lastValidationRequest?.reject();
            const validationRequest = new DxDeferred();
            result.complete.then(asyncResult => {
                validationRequest.resolve(asyncResult);
            }, () => {
                validationRequest.resolve({ isValid: false });
            });
            validationRequest.done(asyncResult => { this._handleValidationResult(asyncResult); });
            this.lastValidationRequest = validationRequest;
        }
        else {
