...
export class PreviewParameterHelper extends ParameterHelper {
    mapLookUpValues(type, lookUpValues, allowNull) {
        const converter = this.getValueConverter(type, allowNull);
    ...
    createInfo(parameter) {
        ...
        if (parameter.isRange) {
            info.editor = this.getRangeEditor(parameter.type);
            return info;
        }
    ...
    getValueConverter(type, allowNull) {
        switch (type) {
            ...
            case DotnetTypes.SystemInt64:
                return val => integerValueConverter(val, allowNull ? null : '0');
            case DotnetTypes.SystemSingle:
            case DotnetTypes.SystemDouble:
            case DotnetTypes.SystemDecimal:
                return val => floatValueConverter(val, allowNull ? null : '0');
            default:
                return super.getValueConverter(type);
        }
    }
    getRangeEditor(type) {
        return { ...viewerEditorTemplates.rangeEditor, type: type };
    }
