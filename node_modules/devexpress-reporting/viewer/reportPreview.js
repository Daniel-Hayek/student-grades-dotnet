...
export class ReportPreview extends ReportHolder {
    ...
    dispose() {
        ...
        this.previewBrickKeyboardHelper?.dispose?.();
    }
    removePage(page) {
    ...
    _initialize(closeDocument = true, closeReport = false) {
        ...
        if (closeDocument) {
            const _reportId = closeReport ? this.reportId : null;
            this.closeDocumentAndReport(this.documentId, _reportId);
        }
        this._editingFields = [];
            ...
            const defaultHandler = () => {
                if (brickNavigation) {
                    if (brickNavigation.drillThroughData && _self.reportId) {
                        ...
                        const lowercasedUrl = (typeof brickNavigation.url === 'string' && brickNavigation.urlChecked) ? brickNavigation.url.toLowerCase() : null;
                        if (lowercasedUrl && lowercasedUrl !== 'empty') {
    ...
    constructor(handlerUri, previewRequestWrapper, previewHandlersHelper, callbacks, rtl = false, enableKeyboardSupport, exportSettings, element, breadcrumb) {
        ...
        this._exportTranslatedDodumentWarningMsg = getLocalization('You are about to export a document that contains AI-generated translations. Do you want to proceed?', 'ASPxReportsStringId.WebDocumentViewer_AI_ExportDocumentWithTranslations_Message');
        this._exportTranslatedDocumentButtonText = getLocalization('OK', 'ASPxReportsStringId.WebDocumentViewer_AI_ExportDocumentWithTranslations_OK');
        this._exportOriginalDocumentButtonText = getLocalization('Cancel', 'ASPxReportsStringId.WebDocumentViewer_AI_ExportDocumentWithTranslations_Cancel');
        ...
        this._onDocumentBuildingChanged = (newVal) => {
            if (!newVal) {
                ...
                for (let i = 0; i < pageCount; i++) {
                    const page = this.pages[i];
                    if (!page.pageLoading) {
                        page.bricks?.length && page.clearBricks();
                    }
    ...
    drillThrough(drillThroughData) {
        if (this._currentDrillThroughPromise) {
            return this._currentDrillThroughPromise;
        }
        const deferred = new DxDeferred();
        const promise = deferred.promise();
        this._deferreds.push(deferred);
        this._currentDrillThroughPromise = promise;
    ...
    _documentHasTranslation() {
        return this.pages?.some(page => !!page.aiCustomDocumentStorageId) ?? false;
    }
    ...
    exportDocumentTo(format, inlineResult) {
        ...
        const serializedExportOptions = this.exportOptionsModel ? JSON.stringify(nativeModelSerializer().serialize(this.exportOptionsModel)) : null;
        const customBrickDataStorages = this.pages?.map(page => page.aiCustomDocumentStorageId).filter(Boolean);
        const args = () => JSON.stringify({
            ...
            customBrickDataStorageIds: customBrickDataStorages
        });
    ...
    closeDocument(documentId) {
        const _documentId = this.handleDocumentClose(documentId);
        if (!_documentId)
            return;
        this._closeDocumentRequests[_documentId] = true;
        this.requestWrapper.sendCloseRequest(_documentId);
    }
    closeDocumentAndReport(documentId, reportId) {
        const _documentId = this.handleDocumentClose(documentId);
        if (!_documentId && !reportId)
            return;
        if (_documentId) {
            this._closeDocumentRequests[_documentId] = true;
        }
        if (reportId) {
            this._closeReportRequests[reportId] = true;
        }
        this.requestWrapper.sendCloseRequest(_documentId, reportId);
    }
