...
export class TimeRangeDialogElementsKeyboardHelper extends AccessibilityKeyboardHelperBase {
    constructor(_timeRangeEditor) {
        super();
        this._timeRangeEditor = _timeRangeEditor;
        this.controlElementClassName = 'dx-accessibility-timerange-item';
    }
    createControlElement(element, index) {
        return new AccessibilityControlElementBase((this.getContainer().getElementsByClassName(this.controlElementClassName)[index]));
    }
    itemHandleEscKey(e, index) {
        this._timeRangeEditor?._hidePopup();
        return true;
    }
    clearSelection() {
        document.querySelectorAll(`.${this.controlElementClassName}`).forEach(element => { element.classList.remove('dxrv-predefined-timerange-active'); });
    }
    changeSelections(item) {
        document.querySelectorAll(`.${this.controlElementClassName}`).forEach(element => { element.classList.remove('dxrv-predefined-timerange-active'); });
        setTimeout(() => {
            const itemElement = document.querySelector(`#${item?.id}`);
            itemElement?.classList.add('dxrv-predefined-timerange-active');
            itemElement?.focus();
        }, 10);
    }
}
